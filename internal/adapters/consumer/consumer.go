...
func (c *Consumer) Start(ctx context.Context)  {
	slog.Info("starting batch consumer", "topic", c.cfg.Topic, "workers", c.cfg.WorkersCount)
	...
	defer func() {
        slog.Info("stopping consumer: closing worker channels from producer")
	...
	for {
		...
		if err != nil {
			if errors.Is(err, context.Canceled) {
				slog.Info("context canceled, stopping fetch loop")
				break
			}
		...
		if err := json.Unmarshal(msg.Value, &event); err != nil {
			slog.Error("poison pill: unmarshal failed", "err", err, "offset", msg.Offset)
			c.handlePoisonPill(msg, tracker, fmt.Errorf("unmarshal error: %w", err))
			continue
		}

		if err := c.validate.Struct(event); err != nil {
			slog.Error("poison pill: validation failed", "err", err, "offset", msg.Offset)
		...
		if err != nil {
			slog.Error("poison pill: invalid time format", "err", err, "offset", msg.Offset)
...
func (c *Consumer) processAndCommitBatch(ctx context.Context, batchTx []*domain.Transaction, batchMsg []kafka.Message, workerID int) bool {
    ...
    for attempts < maxAttempts {
        if ctx.Err() != nil {
            slog.Warn("context canceled during batch process, deferring to shutdown drain", "worker", workerID)
            return false
        }
        ...
        attempts++
        lastErr = err
        slog.Error("batch save failed, retrying", "err", err, "attempt", attempts, "worker", workerID)
    ...
    if lastErr != nil && !batchSaved {
        slog.Error("max retries reached, routing entire batch to DLQ", "worker", workerID)

        if ctx.Err() != nil {
            slog.Warn("context canceled before routing to DLQ, deferring to shutdown drain", "worker", workerID)
            return false 
        }
    ...
    slog.Error("Batch was neither saved to DB nor DLQ. Offsets WILL NOT be committed.", 
        "worker", workerID, 
...
func (c *Consumer) sendToDlq(ctx context.Context, originalErr error, msgs ...kafka.Message) error {
    if c.errorHandler == nil {
        slog.Warn("DLQ handler not configured, skipping messages (DATA LOSS)", "count", len(msgs))
    ...
    for i := 0; i < c.cfg.MaxDlqRetries; i++ {
        ...
        slog.Error("CRITICAL: failed to write to DLQ, retrying", 
            "err", err, 
            "count", len(msgs),
            "attempt", i+1,
        )
    ...
    slog.Error("FATAL: DATA LOSS. Failed to write to DB and DLQ", "count", len(msgs))
...
func (c *Consumer) commitProgress(ctx context.Context, msg kafka.Message, tracker *PartitionWatermark) {
	...
	if shouldCommit {
		...
		if err != nil {
			slog.Error("failed to commit offset", "err", err, "partition", msg.Partition, "offset", msg.Offset)
		}
	}
}
...
func (c *Consumer) Stop(ctx context.Context) error {
    ...
    slog.Info("waiting for workers to finish in-flight batches...")
    ...
    select {
    case <-done:
        slog.Info("all workers finished successfully")
    case <-ctx.Done():
        shutdownErr = fmt.Errorf("consumer shutdown timeout exceeded: %w", ctx.Err())
        slog.Error("CRITICAL: partial shutdown! some in-flight batches were dropped", "err", shutdownErr)
    }
